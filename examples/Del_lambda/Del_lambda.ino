...
DelPin maDel(PIN); // Initialisation de la DEL A sur le port choisi

void setup()
{
  maDel.setBlinking(true); //Activer le mode clignotement pour la DEL
  maDel.setBrightness(100);
  maDel.setUpdateFunction([](float percentage) {Serial.println("Ma del est à un pourcentage de "+ String(percentage)) ;}); //Définit la fonction de mise à jour de la DEL
}

void loop()
{
  maDel.refresh(); //Permet d'actualiser l'état de la DEL A. CETTE MÉTHODE EST OBLIGATOIRE EN DÉBUT DE LOOP(), IDÉALEMENT.
